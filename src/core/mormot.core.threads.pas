...
type
  ...
  public
    ...
    constructor Create(aArrayTypeInfo: PRttiInfo; const aName: RawUtf8 = '';
      aKind: TRttiParserType = ptNone; aCaseInsensitive: boolean = false;
      noKindDetect: boolean = false); reintroduce; virtual;
    {$ifdef HASGENERICS}
...
constructor TSynQueue.Create(aArrayTypeInfo: PRttiInfo; const aName: RawUtf8;
  aKind: TRttiParserType; aCaseInsensitive: boolean; noKindDetect: boolean);
begin
  inherited Create(aName);
  fFirst := -1;
  fLast := -2;
  fValues.Init(aArrayTypeInfo, fValueVar, @fCount);
  if (aKind = ptNone) and not noKindDetect then
